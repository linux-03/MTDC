...
class Data:
    ...
    def _get_prices(self):
        currency_pairs = ['USDJPY', 'EURCHF', 'EURJPY', 'EURUSD', 'USDCAD', 'USDCHF', 'USDJPY']
        #data = pd.read_csv('data/AUDCHF_historical_data.csv')
        data = pd.concat([
            pd.read_csv(f'data/{pair}-2000-2020-15m.csv', parse_dates=['DATE_TIME']).assign(CURRENCY_PAIR=pair)
            for pair in currency_pairs
        ])
        """ data = pd.concat([
            pd.read_csv(f'data/{pair}-1016-2024.csv', engine='python', sep='\\t', parse_dates=['DATE_TIME']).assign(CURRENCY_PAIR=pair)
            for pair in currency_pairs
        ]) """
        #print(data.head())
